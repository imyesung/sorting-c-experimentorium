...
* Quadratic comparison sorts (`O(n²)` in the general case):
...
* Run without very large sizes (faster, good for quick iterations):

```bash
./bin/benchmark --no-large-sizes
```

* Regenerate PNG plots from existing CSV only (no re-benchmarking):
...
On a linear y-axis, `O(n²)` behavior becomes much more dramatic.
...
The three pattern-comparison PNGs show how input ordering affects each algorithm family.
